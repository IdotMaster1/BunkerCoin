...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total Bunkercoin balance of the wallet\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"bunkercoinaddress\"\n"
            "\nReturn information about the given bunkercoin address.\n"
            "\nArguments:\n"
            "1. \"bunkercoinaddress\"     (string, required) The bunkercoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"bunkercoinaddress\", (string) The bunkercoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Bunkercoin address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are bunkercoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) bunkercoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"bunkercoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"bunkercoinaddress\"  (string, required) The bunkercoin address to use for the signature.\n"
